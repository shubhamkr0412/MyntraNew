...
const MensPage = () => {
  ...
  const getMensData = (page = 1, Category = null, price = null) => {
    if (!Category && !price) {
      // console.log(page);
      fetch(`https://mmyntraclone.herokuapp.com/men?_page=${page}&_limit=9`)
        .then((d) => d.json())
        .then((res) => {
          setLists(res);
        });
      ...
      fetch(`https://mmyntraclone.herokuapp.com/men?Category=${Category}`)
        .then((d) => d.json())
        .then((res) => {
          setLists(res);
        });
      ...
      fetch(`https://mmyntraclone.herokuapp.com/men`)
        .then((d) => d.json())
        .then((res) => {
